...
static const TagMapEntry TAG_TYPES_BY_TAG_NAME[] = {
    ...
    {"ISCOOKIE", ISCOOKIE},
    {"ISSTATUS", ISSTATUS},
    {"ISACTIVEDATAHEAD", ISACTIVEDATAHEAD},
    {"ISACTIVEDATACONTEXT", ISACTIVEDATACONTEXT},
    {"ISANALYTICSOFF", ISANALYTICSOFF},
...
static TagType tag_type_for_name(const String *tag_name) {
    for (size_t i = 0; i < sizeof(TAG_TYPES_BY_TAG_NAME) / sizeof(TagMapEntry); i++) {
...
static inline bool tag_is_raw_text(const Tag *self) {
    return self->type == SCRIPT || self->type == STYLE ||
           self->type == ISSCRIPT || self->type == ISCOMMENT;
}
